...
import com.draekko.traypreferences.util.CharSequences;

import com.draekko.traypreferences.annotations.CallSuper;
import com.draekko.traypreferences.annotations.DrawableRes;
import com.draekko.traypreferences.annotations.LayoutRes;
 ...
 * block displayed by a {@link TrayPreferenceActivity} in the form of a
 * {@link ListView}. This class provides the {@link View} to be displayed in
 * the activity and associates with a {@link TraySharedPreferences} to
 * store/retrieve the preference data.
 * <p>
 * When specifying a preference hierarchy in XML, each element can point to a
 * subclass of {@link TrayPreference}, similar to the view hierarchy and layouts.
 * <p>
 * This class contains a {@code key} that will be used as the key into the
 * {@link TraySharedPreferences}. It is up to the subclass to decide how to store
 * the value.
 * 
 ...
 * </div>
 * 
 * @attr ref R.styleable#TrayPreferences_icon
 * @attr ref R.styleable#TrayPreferences_key
 * @attr ref R.styleable#TrayPreferences_title
 * @attr ref R.styleable#TrayPreferences_summary
 * @attr ref R.styleable#TrayPreferences_order
 * @attr ref R.styleable#TrayPreferences_fragment
 * @attr ref R.styleable#TrayPreferences_layout
 * @attr ref R.styleable#TrayPreferences_widgetLayout
 * @attr ref R.styleable#TrayPreferences_enabled
 * @attr ref R.styleable#TrayPreferences_selectable
 * @attr ref R.styleable#TrayPreferences_dependency
 * @attr ref R.styleable#TrayPreferences_persistent
 * @attr ref R.styleable#TrayPreferences_defaultValue
 * @attr ref R.styleable#TrayPreferences_shouldDisableView
 */
public class TrayPreference implements Comparable<TrayPreference> {
    /**
    ...
    private TrayPreferenceManager mPreferenceManager;
    ...
    private int mLayoutResId = R.layout.traypreference_material;
    private int mWidgetLayoutResId;
    private boolean mCanRecycleLayout = true;
    
    private OnPreferenceChangeInternalListener mListener;
    
    private List<TrayPreference> mDependents;

    TraySharedPreferences traySharedPreferences;
    
    private boolean mBaseMethodCalled;
    
    /**
     * Interface definition for a callback to be invoked when the value of this
     * {@link TrayPreference} has been changed by the user and is
    ...
    public interface OnPreferenceChangeListener {
         ...
         */
        boolean onPreferenceChange(TrayPreference preference, Object newValue);
    }

    /**
     * Interface definition for a callback to be invoked when a {@link TrayPreference} is
     * clicked.
     */
    public interface OnPreferenceClickListener {
        /**
         * Called when a TrayPreference has been clicked.
         *
         * @param preference The TrayPreference that was clicked.
         * @return True if the click was handled.
         */
        boolean onPreferenceClick(TrayPreference preference);
    }

    /**
     * Interface definition for a callback to be invoked when this
     * {@link TrayPreference} is changed or, if this is a group, there is an
     * addition/removal of {@link TrayPreference}(s). This is used internally.
     */
    interface OnPreferenceChangeInternalListener {
        /**
         * Called when this TrayPreference has changed.
         * 
         * @param preference This preference.
         */
        void onPreferenceChange(TrayPreference preference);
        
        /**
         * Called when this group has added/removed {@link TrayPreference}(s).
         * 
         * @param preference This Preference.
         */
        void onPreferenceHierarchyChange(TrayPreference preference);
    }

    /**
     * Perform inflation from XML and apply a class-specific base style. This
     * constructor of TrayPreference allows subclasses to use their own base style
     * when they are inflating. For example, a {@link TrayCheckBoxPreference}
     * constructor calls this version of the super class constructor and
     * supplies {@code R.attr.checkBoxPreferenceStyle} for
     ...
     * {@link TrayCheckBoxPreference} class's attributes.
     *
     * @param context The Context this is associated with, through which it can
     *            access the current theme, resources,
     *            {@link TraySharedPreferences}, etc.
     ...
     * @see #TrayPreference(Context, AttributeSet)
     */
    public TrayPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        mContext = context;

        traySharedPreferences = new TraySharedPreferencesImpl(mContext);

        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.TrayPreferences, defStyleAttr, defStyleRes);
        for (int i = a.getIndexCount() - 1; i >= 0; i--) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.TrayPreferences_icon) {
                mIconResId = a.getResourceId(attr, 0);
            } else if (attr == R.styleable.TrayPreferences_key) {
                mKey = a.getString(attr);
            } else if (attr == R.styleable.TrayPreferences_title) {
                mTitleRes = a.getResourceId(attr, 0);
                mTitle = a.getString(attr);
            } else if (attr == R.styleable.TrayPreferences_summary) {
                mSummary = a.getString(attr);
            } else if (attr == R.styleable.TrayPreferences_order) {
                mOrder = a.getInt(attr, mOrder);
            } else if (attr == R.styleable.TrayPreferences_fragment) {
                mFragment = a.getString(attr);
            } else if (attr == R.styleable.TrayPreferences_layout) {
                mLayoutResId = a.getResourceId(attr, mLayoutResId);
            } else if (attr == R.styleable.TrayPreferences_widgetLayout) {
                mWidgetLayoutResId = a.getResourceId(attr, mWidgetLayoutResId);
            } else if (attr == R.styleable.TrayPreferences_enabled) {
                mEnabled = a.getBoolean(attr, true);
            } else if (attr == R.styleable.TrayPreferences_selectable) {
                mSelectable = a.getBoolean(attr, true);
            } else if (attr == R.styleable.TrayPreferences_persistent) {
                mPersistent = a.getBoolean(attr, mPersistent);
            } else if (attr == R.styleable.TrayPreferences_dependency) {
                mDependencyKey = a.getString(attr);
            } else if (attr == R.styleable.TrayPreferences_defaultValue) {
                mDefaultValue = onGetDefaultValue(a, attr);
            } else if (attr == R.styleable.TrayPreferences_shouldDisableView) {
     ...
     * @see #TrayPreference(Context, AttributeSet)
     */
    public TrayPreference(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * 
     * @param context The Context this is associated with, through which it can
     *            access the current theme, resources, {@link TraySharedPreferences},
     *            etc.
     * @param attrs The attributes of the XML tag that is inflating the
     *            preference.
     * @see #TrayPreference(Context, AttributeSet, int)
     */
    public TrayPreference(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.trayPreferenceStyle);
        mContext = context;
    }
     ...
     */
    public TrayPreference(Context context) {
        this(context, null);
        mContext = context;
    }
     ...
     * is inflated into the main layout. For example, a {@link TrayCheckBoxPreference}
    ...
    /**
     * Gets the View that will be shown in the {@link TrayPreferenceActivity}.
     * 
     ...
     * {@link TrayPreferenceActivity}. The default behavior is to inflate the main
     ...
     * {@link TrayPreferenceGroup#setOrderingAsAdded(boolean)} can be used to order
     ...
     * @see TrayPreferenceGroup#setOrderingAsAdded(boolean)
     * @see #DEFAULT_ORDER
     */
     ...
     * the {@link TraySharedPreferences}. However, the overridden method should
     ...
     * {@link TraySharedPreferences}. This should be unique for the package.
     * 
     ...
     * values into TraySharedPreferences.
     * 
     * @return The key.
     */
     ...
     * the {@link TraySharedPreferences}.
     * 
     ...
     * the persistent {@link TraySharedPreferences} storage.
     * 
     ...
     * {@link TraySharedPreferences}. This, at minimum, checks whether this
     * Preference is persistent and it currently has a key. Before you
     * save/restore from the {@link TraySharedPreferences}, check this first.
     * 
     ...
     * it stores its value(s) into the persistent {@link TraySharedPreferences}
     * storage.
     * 
     * @param persistent Set true if it should store its value(s) into the {@link TraySharedPreferences}.
     */
     ...
     * 
     * @param preferenceScreen A {@link TrayPreferenceScreen} whose hierarchy click
     *            listener should be called in the proper order (between other
     *            processing). May be null.
     * @hide
     */
    public void performClick(TrayPreferenceScreen preferenceScreen) {
        ...
        TrayPreferenceManager preferenceManager = getPreferenceManager();
        if (preferenceManager != null) {
            TrayPreferenceManager.OnPreferenceTreeClickListener listener = preferenceManager
     ...
     * {@link TrayPreferenceActivity}). This Context will be used to save the Preference values.
     * 
    ...
    /**
     * Returns the {@link TraySharedPreferences} where this Preference can read its
     ...
     * <p>
     *
     * @return The {@link TraySharedPreferences} where this Preference reads its
     *         value(s), or null if it isn't attached to a Preference hierarchy.
     */
    public TraySharedPreferences getSharedPreferences() {
        TraySharedPreferences traySharedPreferences = null;
        try {
            if (mPreferenceManager != null) {
                traySharedPreferences = mPreferenceManager.getSharedPreferences();
            }
            if (traySharedPreferences == null) {
                traySharedPreferences = TrayPreferenceManager.newSharedPreferences(mContext);
            }
        } catch (Exception e) {
            e.printStackTrace();
            if (traySharedPreferences == null) {
     ...
     */
    @Override
    public int compareTo(TrayPreference another) {
    ...
    /**
     * Should be called when the data of this {@link TrayPreference} has changed.
     */
    ...
    /**
     * Gets the {@link TrayPreferenceManager} that manages this Preference object's tree.
     * 
     * @return The {@link TrayPreferenceManager}.
     */
    public TrayPreferenceManager getPreferenceManager() {
     ...
     */
    protected void onAttachedToHierarchy(TrayPreferenceManager preferenceManager) {
     ...
     * {@link TrayPreferenceActivity}. This can also be called when this
     * Preference has been attached to a group that was already attached
     * to the {@link TrayPreferenceActivity}.
     */
    ...
    private void registerDependency() {
        ...
        TrayPreference preference = findPreferenceInHierarchy(mDependencyKey);
        if (preference != null) {
    ...
    private void unregisterDependency() {
        if (mDependencyKey != null) {
            final TrayPreference oldDependency = findPreferenceInHierarchy(mDependencyKey);
     ...
     * even above/below your {@link TrayPreferenceScreen} screen break) with the given
     * key.
     * <p>
     ...
     */
    protected TrayPreference findPreferenceInHierarchy(String key) {
     ...
     * {@link TrayPreference#setDependency(String)} on the dependent Preference.
     * 
     * @param dependent The dependent Preference that will be enabled/disabled
     *            according to the state of this Preference.
     */
    private void registerDependent(TrayPreference dependent) {
        if (mDependents == null) {
            mDependents = new ArrayList<TrayPreference>();
        }
     ...
     *         into a single statement.
     */
    private void unregisterDependent(TrayPreference dependent) {
        if (mDependents != null) {
    ...
    public void notifyDependencyChange(boolean disableDependents) {
        final List<TrayPreference> dependents = mDependents;
     ...
     */
    public void onDependencyChanged(TrayPreference dependency, boolean disableDependent) {
     ...
     */
    public void onParentChanged(TrayPreference parent, boolean disableChild) {
     ...
     * Preference value from the {@link TraySharedPreferences}. If 
    ...
    /**
     * Attempts to persist a String to the {@link TraySharedPreferences}.
     * <p>
     * This will check if this Preference is persistent, get an editor from
     * the {@link TrayPreferenceManager}, put in the string, and check if we should commit (and
     * commit if so).
     * 
    ...
    protected boolean persistString(String value) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putString(mKey, value);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to get a persisted String from the {@link TraySharedPreferences}.
     * <p>
     * This will check if this Preference is persistent, get the SharedPreferences
     * from the {@link TrayPreferenceManager}, and get the value.
     * 
    ...
    /**
     * Attempts to persist a set of Strings to the {@link TraySharedPreferences}.
     * <p>
     * This will check if this Preference is persistent, get an editor from
     * the {@link TrayPreferenceManager}, put in the strings, and check if we should commit (and
     * commit if so).
     * 
    ...
    public boolean persistStringSet(Set<String> values) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putStringSet(mKey, values);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to persist an int to the {@link TraySharedPreferences}.
     * 
     * @param value The value to persist.
     * @return True if the Preference is persistent. (This is not whether the
     *         value was persisted, since we may not necessarily commit if there
    ...
    protected boolean persistInt(int value) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putInt(mKey, value);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to get a persisted int from the {@link TraySharedPreferences}.
     * 
    ...
    /**
     * Attempts to persist a float to the {@link TraySharedPreferences}.
     * 
     * @param value The value to persist.
    ...
    protected boolean persistFloat(float value) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putFloat(mKey, value);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to get a persisted float from the {@link TraySharedPreferences}.
     * 
     * @param defaultReturnValue The default value to return if either this
     *            Preference is not persistent or this Preference is not in the
     *            SharedPreferences.
    ...
    /**
     * Attempts to persist a long to the {@link TraySharedPreferences}.
     * 
     * @param value The value to persist.
     * @return True if this Preference is persistent. (This is not whether the
     *         value was persisted, since we may not necessarily commit if there
    ...
    protected boolean persistLong(long value) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putLong(mKey, value);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to get a persisted long from the {@link TraySharedPreferences}.
     * 
     * @param defaultReturnValue The default value to return if either this
     *            Preference is not persistent or this Preference is not in the
     *            SharedPreferences.
    ...
    /**
     * Attempts to persist a boolean to the {@link TraySharedPreferences}.
     * 
     * @param value The value to persist.
     * @return True if this Preference is persistent. (This is not whether the
     *         value was persisted, since we may not necessarily commit if there
    ...
    protected boolean persistBoolean(boolean value) {
        if (shouldPersist()) {
            ...
            traySharedPreferences.putBoolean(mKey, value);
            return true;
        }
        return false;
    }
    ...
    /**
     * Attempts to get a persisted boolean from the {@link TraySharedPreferences}.
     * 
     * @param defaultReturnValue The default value to return if either this
     *            Preference is not persistent or this Preference is not in the
     *            SharedPreferences.
    ...
    /**
     * A base class for managing the instance state of a {@link TrayPreference}.
     */
